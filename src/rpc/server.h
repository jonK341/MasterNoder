...
/**
 * MasterNoder RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZmnSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzmnseed(const UniValue& params, bool fHelp);
extern UniValue getzmnseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzmn(const UniValue& params, bool fHelp);
extern UniValue dzmnstate(const UniValue& params, bool fHelp);
