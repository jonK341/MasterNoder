...
CzMNWallet::CzMNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMNSeed_deprecated()) {
                    LogPrintf("%s: Updated zMN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMNCount(nCountLastUsed);
    else if (!walletdb.ReadZMNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMNWallet::Lock()
{
    seedMaster = 0;
}

void CzMNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMNWallet::LoadMintPoolFromDB()
{
...
void CzMNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmnTracker which also adds to database
    pwalletMain->zmnTracker->Add(dMint, true);
...
void CzMNWallet::SeedToZMN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMNWallet::GenerateDeterministicZMN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMNWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzMNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
