...
CzMNTracker::CzMNTracker(std::string strWalletFile)
{
...
CzMNTracker::~CzMNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMNTracker::Init()
{
...
bool CzMNTracker::Archive(CMintMeta& meta)
{
...
bool CzMNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzMNTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzMNTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzMNTracker::GetSerialHashes()
{
...
CAmount CzMNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzMNWallet* zMNWallet)
{
    bool iszMNWalletInitialized = (NULL != zMNWallet);
    CMintMeta meta;
    ...
    if (! iszMNWalletInitialized)
        zMNWallet = new CzMNWallet(strWalletFile);
    meta.isSeedCorrect = zMNWallet->CheckSeed(dMint);
    if (! iszMNWalletInitialized)
        delete zMNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzMNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzMNWallet* zMNWallet = new CzMNWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zMNWallet);
        }
        delete zMNWallet;
        LogPrint("zero", "%s: added %d dzmn from DB\n", __func__, listDeterministicDB.size());
    }
