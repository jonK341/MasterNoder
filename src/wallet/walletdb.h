...
public:
    ...
    bool WriteZMNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMNSeed_deprecated(uint256& seed);
    bool EraseZMNSeed();
    bool EraseZMNSeed_deprecated();

    bool WriteZMNCount(const uint32_t& nCount);
    bool ReadZMNCount(uint32_t& nCount);
