...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zmnTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zmnTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zMN.";
            else if (!mint.isSeedCorrect)
                strReason = "The zMN seed used to mint this zMN is not the same as currently hold in the wallet";
            else
