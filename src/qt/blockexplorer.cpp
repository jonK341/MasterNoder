...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::MN, nValue);
...
{
    std::string genesisblockhash = "00000a25e45b2fbc00be550b2fdc8017d016be06889e06fb5d03468e5ff39228";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (masternoder.conf).");
            QMessageBox::warning(this, "MasterNoder Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
