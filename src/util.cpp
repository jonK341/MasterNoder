...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "masternoder" is a composite category enabling all MasterNoder-related debug output
            if (ptrCategory->count(string("masternoder"))) {
...
#else
    const char* pszModule = "masternoder";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\MasterNoder
// Windows >= Vista: C:\Users\Username\AppData\Roaming\MasterNoder
// Mac: ~/Library/Application Support/MasterNoder
// Unix: ~/.masternoder
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MasterNoder";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "masternoder.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override masternoder.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "masternoderd.pid"));
