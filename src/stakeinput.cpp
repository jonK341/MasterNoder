...
// The zMN block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zMN is a hash of the serial
...
{
    //Create an output returning the zMN that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZMNOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zMN output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zMN", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZMNOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zMN output", __func__);
...
{
    CzMNTracker* zmnTracker = pwallet->zmnTracker.get();
    CMintMeta meta;
    if (!zmnTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zmnTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a MN stake is the outpoint
